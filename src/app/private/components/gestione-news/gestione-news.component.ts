...
export class GestioneNewsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'titolo', 'autore', 'testo', 'dettagli'];
  ...
  populateTable(result: PaginationResult<News>) {
    this.dataSource.data = result.items.map((item: any) => new News(item))
  ...
  save() {
    ...
    this.newsService.save(raw).subscribe(res => {
      this.utilsService.notificationsService.success('News inserita correttamente');
      this.search(true);
      // if (!raw.id) {
      //   this.timelineService.save({
      //     tipo_timeline: 'Arrivo',
      //     data: new Date(),
      //     descrizione: 'Arrivo News',
      //     id_News: res.id,
  ...
  delete() {
    let raw = this.editItemForm.getRawValue();
    this.newsService.delete(raw.id).subscribe(res => {
      this.utilsService.notificationsService.success('News eliminata con successo');
      this.search(true);
